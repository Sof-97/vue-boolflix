<template>
	<div id="app">
		...
		<main class="container mt-3">
			...
			<h5 class="no-result" v-if="this.filmData.length == 0">
				Nessun risultato per film
			</h5>
			<h2 v-if="selected.text != ''" class="mt-4">Serie TV</h2>
			<ShowComp :showData="showData" />
			<h5
				class="no-result"
				v-if="this.showData.length == 0 && this.selected.text != ''"
			>
...
export default {
	...
	data() {
		return {
			...
			popular:
				"https://api.themoviedb.org/3/trending/movie/day?api_key=45fcb174ff225f6bfb45caa448731eef",
			selected: {
				lang: "it-IT",
				text: "",
				genre: "",
